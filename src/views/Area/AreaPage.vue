<template>
  <section class="area">
    <div class="container">
      <h2 class="area__name title">{{ area[1] }}</h2>

      <div class="meals__cards">
        <div class="meals__card" v-for="(meal, idx) in area[0]" :key="idx">
          ...
          <div class="meals__about">
            ...
            <router-link
              ...
              class="meals__more"
              @click="$store.commit('setId', meal)"
              >More</router-link
            >

            <!-- can't find mealPage ing. (store. 73-79) -->
          </div>
        </div>
      </div>
    </div>
